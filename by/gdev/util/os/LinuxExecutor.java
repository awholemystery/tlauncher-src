...
/* loaded from: TLauncher-2.876.jar:by/gdev/util/os/LinuxExecutor.class */
public class LinuxExecutor implements OSExecutor {
    ...
    public GPUDriverVersion getGPUDriverVersion() throws IOException {
        ...
        String[] res = s.split(" ");
        if (res.length == 3) {
            Optional<GPUDriverVersion> op = Arrays.stream(GPUDriverVersion.values()).filter(f -> {
                return res[2].startsWith(f.getValue());
            }).findFirst();
            if (op.isPresent()) {
                return op.get();
            }
    ...
    public GPUsDescriptionDTO getGPUInfo1(String res, String stringStart) {
        String[] params = res.split(System.lineSeparator());
        List<GPUDescription> gpus = (List) Arrays.stream(params).map((v0) -> {
            return v0.toLowerCase();
        }).filter(e -> {
            return e.contains(stringStart);
        }).map(s -> {
