...
@NotThreadSafe
/* loaded from: TLauncher-2.876.jar:org/apache/http/impl/auth/DigestScheme.class */
    ...
    private Header createDigestHeader(Credentials credentials, HttpRequest request) throws AuthenticationException {
        ...
        if (qoplist != null) {
            ...
            if ((request instanceof HttpEntityEnclosingRequest) && qopset.contains("auth-int")) {
                qop = 1;
            } else if (qopset.contains(LoginForm.AUTH_BLOCK)) {
                qop = 2;
            }
        } else {
            qop = 0;
        ...
        try {
            ...
            if (qop == 2) {
                ...
                if (entity != null && !entity.isRepeatable()) {
                    if (qopset.contains(LoginForm.AUTH_BLOCK)) {
                        qop = 2;
                        this.a2 = method + ':' + uri;
                    } else {
            ...
            if (qop == 0) {
                ...
                sb.setLength(0);
                sb.append(hasha1).append(':').append(nonce).append(':').append(nc).append(':').append(this.cnonce).append(':').append(qop == 1 ? "auth-int" : LoginForm.AUTH_BLOCK).append(':').append(hasha2);
                digestValue = sb.toString();
            }
            ...
            if (qop != 0) {
                params.add(new BasicNameValuePair("qop", qop == 1 ? "auth-int" : LoginForm.AUTH_BLOCK));
