...
@Immutable
/* loaded from: TLauncher-2.876.jar:org/apache/http/impl/pool/BasicConnFactory.class */
public class BasicConnFactory implements ConnFactory<HttpHost, HttpClientConnection> {
    ...
    public HttpClientConnection create(HttpHost host) throws IOException {
        ...
        if (CachingURLStreamHandlerFactory.PROTO_HTTPS.equalsIgnoreCase(scheme)) {
        ...
        if (port == -1) {
            if (host.getSchemeName().equalsIgnoreCase("http")) {
                port = 80;
            } else if (host.getSchemeName().equalsIgnoreCase(CachingURLStreamHandlerFactory.PROTO_HTTPS)) {
                port = 443;
            }
        }
