...
/* loaded from: TLauncher-2.876.jar:org/apache/log4j/LogMF.class */
public final class LogMF extends LogXF {
    ...
    private static boolean isSimple(String pattern) {
        ...
        int indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT);
        while (true) {
            int pos = indexOf;
            if (pos != -1) {
                if (pos + 2 < pattern.length() && pattern.charAt(pos + 2) == '}' && pattern.charAt(pos + 1) >= '0' && pattern.charAt(pos + 1) <= '9') {
                    indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT, pos + 1);
                } else {
                    return false;
                }
            } else {
    ...
    private static String format(String pattern, Object[] arguments) {
        ...
        if (isSimple(pattern)) {
            ...
            int prev = 0;
            String retval = CoreConstants.EMPTY_STRING;
            int indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT);
            while (true) {
                int pos = indexOf;
                if (pos >= 0) {
                    if (pos + 2 < pattern.length() && pattern.charAt(pos + 2) == '}' && pattern.charAt(pos + 1) >= '0' && pattern.charAt(pos + 1) <= '9') {
                        ...
                        prev = pos + 3;
                        indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT, prev);
                    } else {
                        indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT, pos + 1);
                    }
                } else {
            ...
            while (true) {
                ...
                if (pos >= 0) {
                    if (pos + 2 < pattern.length() && pattern.charAt(pos + 2) == '}' && pattern.charAt(pos + 1) >= '0' && pattern.charAt(pos + 1) <= '9') {
                        ...
                        String retval2 = new StringBuffer().append(retval).append(pattern.substring(prev, pos)).toString();
                        if (index != 0) {
                            stringBuffer = new StringBuffer().append(retval2).append(pattern.substring(pos, pos + 3)).toString();
                        } else {
                            if (formatted == null) {
                                formatted = formatObject(arg0);
                            }
                            stringBuffer = new StringBuffer().append(retval2).append(formatted).toString();
                        }
                        retval = stringBuffer;
                        prev = pos + 3;
                        indexOf = pattern.indexOf(CoreConstants.CURLY_LEFT, prev);
