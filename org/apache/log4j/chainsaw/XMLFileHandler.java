...
/* loaded from: TLauncher-2.876.jar:org/apache/log4j/chainsaw/XMLFileHandler.class */
class XMLFileHandler extends DefaultHandler {
        ...
        if (TAG_EVENT.equals(aQName)) {
            ...
            if (this.mThrowableStrRep.length > 0) {
                ...
                for (int i = 1; i < this.mThrowableStrRep.length; i++) {
                    this.mThrowableStrRep[i] = new StringBuffer().append(SyslogAppender.DEFAULT_STACKTRACE_PATTERN).append(st.nextToken()).toString();
                }
            }
        }
    }
    ...
    public void startElement(String aNamespaceURI, String aLocalName, String aQName, Attributes aAtts) {
        ...
        if (TAG_EVENT.equals(aQName)) {
            ...
            this.mLocationDetails = new StringBuffer().append(aAtts.getValue(Action.CLASS_ATTRIBUTE)).append(".").append(aAtts.getValue("method")).append("(").append(aAtts.getValue(Action.FILE_ATTRIBUTE)).append(":").append(aAtts.getValue("line")).append(")").toString();
        }
    }
